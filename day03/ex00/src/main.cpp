...
int		main(){
	...
	FragTrap frag2("\033[1;32mRich Poorman\033[0m");
	...
	while (true)
	{
		...
		if (randAttacker == FRAG)
		{
			if (randHit == 1)
			{
				frag.vaulthunter_dot_exe("\033[1;32mRich Poorman\033[0m");
				frag2.takeDamage(40);
				if (frag2.curr_hp <= 0)
					break ;
			}
			...
			{
				frag.meleeAttack("\033[1;32mRich Poorman\033[0m");
				frag2.takeDamage(30);
				if (frag2.curr_hp <= 0)
					break ;
			}	
			...
			{
				frag.rangedAttack("\033[1;32mRich Poorman\033[0m");
				frag2.takeDamage(20);
				if (frag2.curr_hp <= 0)
					break ;
			}
		...
		else
		{
			if (randHit == 1)
			{
				frag2.vaulthunter_dot_exe("\033[1;31mGeorge Michael\033[0m");
				frag.takeDamage(40);
				if (frag.curr_hp <= 0)
					break ;
			}
	...
	if (frag.curr_hp <= 0)
	{
		std::cout << "\033[1;31mGeorge Michael\033[0m was a bit too much of a Careless Whisperer, and"
			<< " \033[1;32mRich Poorman\033[0m is the VICTOR with " << frag2.curr_hp <<
			" health left!" << std::endl;
	}
	else
	{
		std::cout << "\033[1;32mRich Poorman\033[0m has reached his sad good-bye. \033[1;31mGeorge Michael\033[0m is the VICTOR"
